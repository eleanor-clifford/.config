...
folder="$HOME/.config/i3/lock-cache"
...
resized_portrait="$folder/resized_portrait.png" # resized image for your resolution

# images to be used as wallpaper
dim="$folder/dim.png" # image with subtle overlay of black
dim_portrait="$folder/dim_portrait.png" # image with subtle overlay of black
blur="$folder/blur.png" # blurred version
dimblur="$folder/dimblur.png"
dimblur_portrait="$folder/dimblur_portrait.png"
...
l_dimblur_portrait="$folder/l_dimblur_portrait.png"
...
case "$1" in
	...
	-l | --lock)
		case "$2" in
			...
			dimblur)
				...
				current_res=$(xdpyinfo | grep dimensions | sed -r 's/^[^0-9]*([0-9]+x[0-9]+).*$/\1/')
        		        IFS='x' read -ra dims <<< "$current_res"
                		if [[ dims[0] -gt dims[1] ]]; then
					lock "$l_dimblur"
				else
					lock "$l_dimblur_portrait"
				fi
				postlock
				;;
		esac
	...
	-u | --update)
		...
		for RES in $SR; do
			...
			CX_INV=$((${SRA[3]} + 25))
			CY_INV=$((${SRA[0]} - 30))
			if [[ SRA[0] -gt SRA[1] ]]; then
				rectangles+="rectangle $CX,$CY $((CX+300)),$((CY-80)) "
				rectangles_portrait+="rectangle $CX_INV,$CY_INV $((CX_INV+300)),$((CY_INV-80)) "
			else
				rectangles_portrait+="rectangle $CX,$CY $((CX+300)),$((CY-80)) "
				rectangles+="rectangle $CX_INV,$CY_INV $((CX_INV+300)),$((CY_INV-80)) "
			fi
		done
		...
		if [[ dims[0] -gt dims[1] ]]; then
			y_res=$current_res
			y_res_portrait=${dims[1]}x${dims[0]}
		else
			y_res_portrait=$current_res
			y_res=${dims[1]}x${dims[0]}			
		fi
		# create folder
		if ! [[ -d $folder ]]; then
			mkdir -p "$folder"
		...
		# resize image
		convert "$orig_wall" -resize "$y_res""^" -gravity west -extent "$y_res" "$resized"
		convert "$orig_wall" -resize "$y_res_portrait""^" -gravity west -extent "$y_res_portrait" "$resized_portrait"

		#echo
		#echo "Applying dim and blur effect to resized image"
		# dim
		convert "$resized" -fill black -colorize 40% "$dim"
		convert "$resized_portrait" -fill black -colorize 40% "$dim_portrait"
		...
		convert "$dim_portrait" -blur 0x5 "$dimblur_portrait"
		...
		convert "$dimblur_portrait" -draw "fill black fill-opacity 0.4 $rectangles_portrait" "$l_dimblur_portrait"
